...
def test_langgraph_agent_seeds_long_term_memory_from_snapshot(tmp_path: Path) -> None:
    """Persist snapshot-derived datasource context into the LangGraph store."""

    database_path = tmp_path / "agent-memory.db"
    engine = create_engine(f"sqlite+pysqlite:///{database_path}")
    with engine.begin() as connection:
        connection.execute(
            text(
        ...
        connection.execute(
            text(
                "INSERT INTO customers (id, email, segment) VALUES (1, 'a@example.com', 'enterprise')"
            )
        )
    engine.dispose()
    ...
    try:
        agent = create_sqldbagent_agent(
            services=container,
            model=ToolReadyFakeMessagesListChatModel(
                responses=[AIMessage(content="memory ok")]
            ),
            datasource_name="sqlite",
            settings=settings,
            schema_name="main",
        ...
        result = agent.invoke(
            {"messages": [{"role": "user", "content": "Say memory ok"}]},
            config={"configurable": {"thread_id": "unit-langgraph-memory"}},
        )
    finally:
        container.close()

    if result["messages"][-1].content != "memory ok":
        raise AssertionError(result)
    ...
    if record is None:
        raise AssertionError("Expected a stored database memory record")
    if record.snapshot_id is None:
        raise AssertionError(record)
    if (
        record.autogenerated_context is None
        or "Table Highlights:" not in record.autogenerated_context
    ):
        raise AssertionError(record)
