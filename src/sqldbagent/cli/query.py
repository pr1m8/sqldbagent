...
@app.command("guard")
def guard(datasource: str, sql: str) -> None:
    ...
    container = build_service_container(datasource)
    try:
        result = container.query_guard.guard(sql)
    finally:
        container.close()
...
@app.command("run")
def run(datasource: str, sql: str, max_rows: int | None = None) -> None:
    ...
    container = build_service_container(datasource)
    try:
        result = container.query_service.run(sql, max_rows=max_rows)
    finally:
        container.close()
...
def run_async(datasource: str, sql: str, max_rows: int | None = None) -> None:
    ...
    result = asyncio.run(_run_async(datasource=datasource, sql=sql, max_rows=max_rows))
    typer.echo(
        orjson.dumps(
